% Context sensitive quotation facilities. Recommended by babel and should be
% loaded before babel.
\usepackage{csquotes}

% Sets the language used. Essential for proper hyphenation. Translates key
% words like 'figure' or 'table'. Note that 'english' is American English.
\usepackage[english]{babel}

% Provides math enviroments, math symbols and more things useful for math.
\usepackage{latexsym, amsmath, amssymb, amsthm, amscd}

% Enhances amsmath and provides further mathematical tools.
\usepackage{mathtools}

% Provides inclusion of graphics and a proper interface for
% '\in­clude­graph­ics'.
% Su­per­sedes 'epsfig' and 'graphics'.
\usepackage{graphicx}

% Provides control over list environments.
% Supersedes 'enumerate', which gives enumerate environment an optional
% argument which determines the style in which the counter is printed.
\usepackage{enumitem}

% Hack to make floatrow work with memoir.
% \let\newfloat\undefined 
% Handles alignment of floats (figures), with centering as default.
% \usepackage{floatrow} 

% Ugly hack that sometimes helps with otherwise missing spaces.
\usepackage{xspace}

% Todo notes.
\usepackage[%
	backgroundcolor=white,
	linecolor=smartblue,
	bordercolor=smartblue,
	textsize=footnotesize]{todonotes}

% Use sans-serif font in todos to clearly distinguish it from other text
\makeatletter
\renewcommand{\todo}[2][]{%
	\@bsphack\@todo[#1]{\sffamily{#2}}\@esphack\ignorespaces}
\makeatother

% Bibliography.
\usepackage[style=alphabetic, backend=biber]{biblatex}

%\usepackage{algorithm} % Algorithms.

% Use default Memoir floats for algorithms
\newcommand{\algorithmname}{Algorithm}
\newcommand{\listalgorithmname}{List of Algorithms}
\newlistof{listofalgorithms}{loa}{\listalgorithmname}
\newfloat[chapter]{algorithm}{loa}{\algorithmname}
\newfixedcaption{\falgcaption}{algorithm}
\newlistentry[chapter]{algorithm}{loa}{0}
\cftsetindents{algorithm}{0em}{2.3em}
\makeatletter
\g@addto@macro\insertchapterspace{\addtocontents{loa}{\protect\addvspace{10pt}}}
\makeatother

% Algorithms.
\usepackage{algpseudocode}

% Automatically locates references on other pages. Load before hyperref.
\usepackage{varioref}

%\usepackage[hidelinks]{hyperref} % Clickable references.
\usepackage[%
	pdfencoding=auto,
	pdfborderstyle={/S/U/W 1},
	linkbordercolor=ref,
	colorlinks,
	linkcolor=ref,
	citecolor=cite,
	urlcolor=url]{hyperref}

% Anchors links to the beginning of their respective floats.
% Load after hyperref.
\usepackage[all]{hypcap}

% Names references automatically. Load after hyperref.
\usepackage[noabbrev, capitalize, nameinlink]{cleveref}

\usepackage[toc, acronym]{glossaries} % Glossary.

\usepackage{booktabs}
\usepackage{multirow}

\usepackage{pgfplots}
\pgfplotsset{compat=1.8}
\pgfplotscreateplotcyclelist{smart}{%
	tangoplum, semithick, every mark/.append style={fill=tangoplum!80!black}, mark=*\\%
	tangogreen, semithick, every mark/.append style={fill=tangogreen!80!black}, mark=square*\\%
	tangoorange, semithick, every mark/.append style={fill=tangoorange!80!black}, mark=otimes*\\%
	tangoblue, semithick, mark=star\\%
	tangobutter, semithick, every mark/.append style={fill=tangobutter!80!black}, mark=diamond*\\%
	tangored, semithick, every mark/.append style={solid, fill=tangored!80!black}, mark=*\\%
}
\pgfplotsset{cycle list name=smart}

